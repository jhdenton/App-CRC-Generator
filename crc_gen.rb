...
class CRC
  ...
  def self.calc_ccitt16(seed, data)
    crc = seed
    #puts "Calculating CRC..."
    data.each do |byte|
      crc = ((crc << 8) ^ CRC_LOOKUP_16[((crc >> 8) ^ byte) & 0xFF]) & 0xFFFF
    end
    #puts "...CRC Done."
    return crc
  end
...
def main
  ...
  if infilename == "?" || infilename == "-help" || infilename == "" || infilename.nil?
    ...
    if !handle.nil?
      ...
      crc_calced = CRC.calc_ccitt16(0x0000, app_memory).to_s(16).upcase
      crc_stored = ((app_crc[1] << 8) + app_crc[0]).to_s(16) .upcase
      ...
      crc_calced = CRC.calc_ccitt16(0x0000, pdi_memory).to_s(16).upcase
      crc_stored = ((pdi_crc[1] << 8) + pdi_crc[0]).to_s(16).upcase
      if crc_calced != crc_stored
